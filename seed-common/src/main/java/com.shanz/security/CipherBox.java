...
import com.shanz.config.AESConfig;
...
public final class CipherBox {
    private AESConfig config  = new AESConfig().build();
    private final int aesKeyLength = config.getAesKeyLength();
    ...
    public RSAPublicKey getPublicKey() {
        if (publicKey == null) {
            String key = config.getPublicKey();
            try {
